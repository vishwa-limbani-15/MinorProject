"""
Hospital HRM Framework - Version 3
==================================

Version 3 builds directly on Version 2.
...
8. Includes a deterministic scenario runner and random-policy smoke test.
...
class DynamicHospitalWorkforceEnvironment:
    ...
    def step(self, action_index: int):
        ...
        reward = self._reward(before, after, changed, valid)
        ...
        info = {
            "date": normalize_date(before_date := pd.Timestamp(self.current_date if self.done else self.episode_dates[max(0, self.current_index - 1)])),
            "action": action,
...
def run_v3_smoke_test(
    ...
    summary = {
        "version": "3",
        "description": "Dynamic staff-level hospital workforce environment smoke test",
    ...
    transitions_path = os.path.join(output_dir, "v3_smoke_test_transitions.json")
    summary_path = os.path.join(output_dir, "v3_environment_summary.json")
    roster_path_out = os.path.join(output_dir, "v3_final_roster_after_smoke_test.csv")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Run Version 3 dynamic hospital workforce environment smoke test.")
